...
def GRASP_v3_iter(model, target_sparsity, batch_size, batchtype):
    eps = 1e-10
    inputs_one = []
    targets_one = []
    ...
    with tf.GradientTape() as tape3:
        for itr in range(num_iters):
            ...
            if grad_w is None:
                ...
                for idx in range(len(grad_w)):
                    grad_w[idx] += grads_w_p[idx]
        for it in range(len(inputs_one)//2):
                
            inputs1 = inputs_one.pop(0)
            targets1 = targets_one.pop(0)

            inputs2 = inputs_one.pop(0)
            targets2 = targets_one.pop(0)
