...
def GRASP_v2(model, target_sparsity, batch_size, batchtype):
   ...
   with tf.GradientTape() as tape3:
      with tf.GradientTape() as tape0, tf.GradientTape() as tape1, tf.GradientTape() as tape2, tf.GradientTape() as tape4:
         tape0.watch(weights)
         ...
         tape2.watch(weights)
         logits3 = model.forward_pass(weights, inputs1)
         loss3 = tf.reduce_mean(compute_loss(targets1, logits3))

         tape4.watch(weights)
         logits4 = model.forward_pass(weights, inputs2)
         loss4 = tf.reduce_mean(compute_loss(targets2, logits4))
      ...
      for idx in range(len(grads_w)):
          grads_w[idx] += tf.stop_gradient(grads_w_p[idx])

      #tape3.watch(weights)
      for idx in range(len(grads_w)):
          z += tf.reduce_sum(grads_w[idx] * grads_f[idx])
      grads_f = tape4.gradient(loss4, [weights[k] for k in prn_keys])
      for idx in range(len(grads_w)):
          z += tf.reduce_sum(grads_w[idx] * grads_f[idx])
